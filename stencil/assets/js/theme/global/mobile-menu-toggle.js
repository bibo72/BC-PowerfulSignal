...
const PLUGIN_KEY1 = 'custom-mobile-menu-toggle';
function optionsFromData($element) {
    const mobileMenuId = $element.data(PLUGIN_KEY);
    const customMobileMenuId = $element.data(PLUGIN_KEY1);
    return {
        menuSelector: mobileMenuId && `#${mobileMenuId}`,
        customMenuSelector: customMobileMenuId && `#${customMobileMenuId}`,
    };
}
/*
...
export class MobileMenuToggle {
    ...
    hide() {
        ...
        this.resetSubMenus();
        setTimeout(() => {
            $('.navPages-container').css('visibility', 'hidden');
        }, 500);
    }
    // Private
    onToggleClick(event) {
